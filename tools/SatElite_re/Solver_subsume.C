...
void Solver::simplifyBySubsumption(bool with_var_elim)
{
    ...
    int     sum_vars_elimed=0;//modified by chen 2013/03/8
...
//Report("  PREPARING\n");
            ...
            for (int i = 0; i < cl_added.size(); i++){
                ...
                for (int j = 0; j < c.size(); j++){
                    ...
                    for (int k = 0; k < n_occs.size(); k++)     // <<= B\E4ttra p\E5. Beh\F6ver bara kolla 'n_occs[k]' mot 'c'
                    ...
                    for (int k = 0; k < p_occs.size(); k++)     // <<= B\E4ttra p\E5. Beh\F6ver bara kolla 'p_occs[k]' mot 'c'
...
//Report("VARIABLE ELIMINIATION\n");
        ...
        for (bool first = true;; first = false){
            ...
            sum_vars_elimed+=vars_elimed;//modified by chen 2013/03/8
        ...
        if(sum_vars_elimed/9>nVars()/10 && sum_vars_elimed>200000) break; //modified by chen 2013/03/8 
	    ...
	    printf("i CPU time: %g\n", cpuTime());
            ...
            for (int i = 0; i < nVars(); i++){
                if (!var_elimed[i] && value(i) != l_Undef) {
                    printf("%s%s%d", (i > 0)?" ":"", (value(i)==l_True)?"":"-", i+1);
                }
            }
            printf(" 0\n");
            exit(0);
...
3 -> ~4       == { ~3, ~4 }   (ger ~4, ~5 + ev mer som superset; negera detta och l\E4gg till 3:an)
3 -> ~5       == { ~3, ~5 }
...
void Solver::clauseReduction(void)
{
    ...
    for (int i = 0; i < constrs.size(); i++){
        ...
        if (analyze){
            ...
            // (kolla att seen[] \E4r nollad korrekt h\E4r)
